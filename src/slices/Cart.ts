...
const initialState = {
  toggleShow: false,
  items: [],
} as { items: any[]; toggleShow: boolean };
  ...
  reducers: {
    ...
    add: (state, action: PayloadAction<IProduct>) => {
      ...
      const existProductIndex = state.items.findIndex(
        (item: IProduct) => item.id == newProduct.id,
      );
    ...
    increase: (state, action: PayloadAction<number>) => {
      state.items.find((item: IProduct) => item.id === action.payload)
        .quantity++;
    },
...
export const { show, hide, add, increase, decrease } = cartSlice.actions;
